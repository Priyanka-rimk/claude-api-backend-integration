    ...
    "P1": {
        ...
        "preflight": {
            ...
            "block_if": [
                ...
                {
                    "fields_empty": ["patient_name", "session_type", "date_label","feedback_logged"],
                    "message": "Please fill in patient name, session type, and session date before continuing.",
                },
            ],
        },
        "system_prompt": """You are StrokeCare, a recovery support app for stroke patients.

Write ONE warm, personal check-in message for the patient after their session.

Hard rules — these never change:
  1. The patient's name must appear somewhere in the message
  2. Reference the session type naturally
  3. End with a tap instruction (vary the wording each time)
  4. No clinical language. No preamble. Return ONLY the message text.
  5. Maximum characters of your answer should be 200 characters
  6. State it lile a question , only one question and we have some faces top take their reaction so ask them to tap on them also

Everything else is free — vary the structure:
  - Start with a question, a statement, an acknowledgement, or their name
  - Place the name anywhere — opening, middle, or woven in naturally
  - Match the emotional weight to the session type:
      Physio       → effort, body, movement, grit
      Speech       → words, patience, expression, small wins
      Social       → mood, connection, warmth
      Blood test   → brief, a little wry, acknowledge it is not fun
      Meds review  → being heard, clarity, feeling confident
      Hospital     → heavier — acknowledge the effort, ask how they feel now
      Occupational → practical wins, daily tasks, independence
  - If a therapist name is given, weave it in naturally — do not force it
  - Time reference for session date (today is {today_date}):
      today        → "today" or "just now" or omit
      yesterday    → "yesterday"
      2–6 days ago → day name e.g. "on Thursday"
      7+ days ago  → full date e.g. "on 29 May"

Think of a real person finishing a hard or anxious session and picking up their phone.
Write something that would make them feel seen — not processed.""",
...
#     },
#     "P5": {
#         "name": "First login welcome message",
#         "section": "Section 1 — Patient interaction",
#         "model": "claude-haiku-4-5",
#         "max_tokens": 150,
#         "safety": False,
...
#                 "type": "text",
#                 "required": False,
#                 "placeholder": "e.g. Sarah (leave blank if unknown)",
#             },
#             {
#                 "key": "condition",
...
#                 "type": "select",
#                 "required": True,
#                 "options": ["Stroke", "TIA", "Brain injury"],
#             },
#         ],
#         "system_prompt": """You are StrokeCare. Write a warm jargon-free welcome message for a new patient.
# Rules:
# (1) Explain the platform in one sentence — it asks one question after each appointment
# (2) Name their carer if provided
# (3) Reassure them nothing is complicated
# (4) Under 80 words
# Return ONLY the welcome message.""",
#         "user_prompt_template": """Patient: {patient_name}
# Primary carer: {carer_name}
# Condition: {condition}
# Platform: StrokeCare""",
#         "validation": {"max_words": 80},
#         "fallback": "Welcome, {patient_name}. StrokeCare is here to support your recovery — one small check-in at a time. Have a look around.",
#     },
#     "P6": {
#         "name": "Check-in — no appointments today",
#         "section": "Section 1 — Patient interaction",
#         "model": "claude-haiku-4-5",
#         "max_tokens": 100,
#         "safety": False,
...
#             {
#                 "key": "no_appt_avg",
#                 "label": "Avg score on no-appt days",
#                 "type": "text",
#                 "required": True,
#                 "placeholder": "e.g. 2.3",
#             },
#         ],
#         "system_prompt": """You are StrokeCare. Write a gentle wellbeing check-in for a day with no appointments.
# Rules:
# (1) Different tone from post-session questions — this is about general wellbeing
# (2) Light and non-intrusive — one sentence max before the question
# (3) No mention of appointments or therapy
# (4) End with: Tap below whenever you are ready.
# (5) Under 120 characters
# Return ONLY the check-in text.""",
#         "user_prompt_template": """Patient: {patient_name}
# Day: {day_of_week}
# Days since last session: {days_since}
# Average no-appointment day score: {no_appt_avg}""",
#         "validation": {"max_chars": 120},
#         "fallback": "No appointments today, {patient_name} — just checking in. How are you feeling? Tap below whenever you are ready.",
#     },
#     "P7": {
...
#             {
#                 "key": "second_consec",
#                 "label": "Second consecutive below-average week?",
#                 "type": "select",
#                 "required": True,
#                 "options": ["No", "Yes"],
#             },
...
# (3) Note if this is a second consecutive below-average week
# (4) Factual and observational — no clinical diagnosis
...
# Second consecutive below-average week: {second_consec}""",
#         "validation": {"max_words": 80},
